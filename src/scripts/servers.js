...
class Servers extends EventEmitter {
    ...
    get hostsKey () {
        return 'floaks.wallet.hosts';
    }

    get activeKey () {
        return 'floaks.wallet.currentHost';
    }
    ...
    setHostTitle (hostUrl, title) {
        if (title === 'Floaks.Wallet' && /https?:\/\/open\.floaks\.wallet/.test(hostUrl) === false) {
            title += ' - ' + hostUrl;
        }
        const hosts = this.hosts;
    ...
    getProtocolUrlFromProcess (args) {
        let site = null;
        if (args.length > 1) {
            const protocolURI = args.find(arg => arg.startsWith('floakswallet://'));
            if (protocolURI) {
